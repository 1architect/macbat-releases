...
demandez : à la recherche de mise à jour (un flux de version sur GitHub) et à
l'activation d'une licence (Gumroad, pour vérifier la clé). Il n'établit aucune
connexion au lancement, ni en arrière-plan.

Vos données restent sur votre Mac, dans `~/Library/Application Support/MacBat/`.
Le détail complet — chaque fichier qu'il écrit, chaque champ qu'il envoie et
comment Sentinelle agit sur les processus — est dans la
[Politique de confidentialité](PRIVACY.fr.md) et la
...
documents ci-dessus. Voir [LICENSE.fr.md](LICENSE.fr.md) pour les termes complets.
